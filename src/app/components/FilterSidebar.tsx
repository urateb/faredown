            ...
            <div className="flex flex-col gap-3 font-medium">
                ...
                {[
                    ...
                    <label key={option.value} className="flex items-center gap-3 cursor-pointer group">
                        <div className={`w-5 h-5 border-2 rounded flex items-center justify-center transition-colors ${stops === option.value ? `${filterBorderColor} bg-gray-700` : `${filterBorderColor} opacity-70 group-hover:opacity-100`}`}>
                            {stops === option.value && <svg className="w-3.5 h-3.5 font-bold text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>}
                        </div>
                        <input
                            type="radio"
                            name="stops"
